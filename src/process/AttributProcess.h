...
#ifndef MURATPROCESSLIB_SRC_AttributProcess_H_
#define MURATPROCESSLIB_SRC_AttributProcess_H_
...
protected:
	...
	QList<std::pair<Seismic3DDataset*, int>> m_attributDatasets;
	private:
		template <typename RgtType>
		struct ComputeModulesKernel {
			static std::vector<std::vector<float>>* run(ToAnalyse5Process* pr,
					...
					const QList<std::pair<Seismic3DDataset*, int>>& attributsDatasets,
...
class AttributProcess : public ToAnalyse5Process {
public:
	AttributProcess(
		Seismic3DDataset *cubeS, int channelS,
		Seismic3DDataset *cubeT, int channelT);
	virtual ~AttributProcess();
