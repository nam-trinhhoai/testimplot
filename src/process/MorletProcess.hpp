...
MorletProcess<InputCubeType>::MorletProcess(
		Seismic3DDataset *cubeS, int channelS,
		Seismic3DDataset *cubeT, int channelT) :
				ToAnalyse3Process( cubeS, channelS, cubeT, channelT) {
	init();
}
...
template <typename InputCubeType>
void MorletProcess<InputCubeType>::init() {

	//io::Cube * cube = nullptr;
}

template <typename InputCubeType>
bool MorletProcess<InputCubeType>::isCompatible(
		e_SliceDirection lastDirection, int lastSlice) {
	return true;//( lastSlice == m_lastSlice && lastDirection == m_lastDirection );
}

template<typename InputCubeType> void MorletProcess<InputCubeType>::saveIn(
