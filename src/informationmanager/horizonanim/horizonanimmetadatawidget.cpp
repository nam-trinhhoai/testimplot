...
HorizonAnimMetadataWidget::HorizonAnimMetadataWidget(IInformation* information, QWidget* parent) :
	...
	if (m_hasFolder) {
		QLabel* pathLabel = new QLabel(mainPath);
		pathLabel->setToolTip(mainPath);
		m_openButton = new QPushButton("Folder");
		mainLayout->addRow(m_openButton, pathLabel);

		connect(m_openButton, &QPushButton::clicked, this, &HorizonAnimMetadataWidget::openDir);
	} else {
		m_openButton = nullptr;
	}
}

HorizonAnimMetadataWidget::~HorizonAnimMetadataWidget() {

}

void HorizonAnimMetadataWidget::openDir() {
	if (!m_hasFolder) {
		return;
	}
