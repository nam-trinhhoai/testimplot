...
NurbInformationMetadataWidget::NurbInformationMetadataWidget(IInformation* information, QWidget* parent) :
	...
	if (m_hasFolder) {
		...
		connect(m_openButton, &QPushButton::clicked, this, &NurbInformationMetadataWidget::openDir);
	} else {
		m_openButton = nullptr;
	}
}

NurbInformationMetadataWidget::~NurbInformationMetadataWidget() {

}

void NurbInformationMetadataWidget::openDir() {
	if (!m_hasFolder) {
		return;
	}
