#ifndef CudaImageBuffer_H
#define CudaImageBuffer_H
...
class CUDAImageBuffer: public QObject {
Q_OBJECT
public:
	CUDAImageBuffer(int width, int height, ImageFormats::QSampleType type,
...
private:
	...
	struct ComputeRangeUnsafeKernel {
		static void run(CUDAImageBuffer* obj, float& x, float& y);
	};
	template<typename InputType>
	struct ComputeHistogramUnsafeKernel {
		static void run(CUDAImageBuffer* obj, unsigned int *hist, const QVector2D &range);
	};
	template<typename InputType>
	struct ByteSwapAndTransposeImageDataKernel {
		static void run(CUDAImageBuffer* obj, void* swappingPointer);
	};
