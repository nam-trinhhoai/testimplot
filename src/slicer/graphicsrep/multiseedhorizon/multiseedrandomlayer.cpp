...
MultiSeedRandomLayer::MultiSeedRandomLayer(MultiSeedRandomRep* rep, QGraphicsScene *scene,
	...
	connect(m_rep->getData(), &MultiSeedHorizon::referencesChanged, this, &MultiSeedRandomLayer::referencesChanged);
}

MultiSeedRandomLayer::~MultiSeedRandomLayer() {
	disconnect(m_rep->getData(), &MultiSeedHorizon::referencesChanged, this, &MultiSeedRandomLayer::referencesChanged);
}

void MultiSeedRandomLayer::show() {
	if(isShown) {
...
void MultiSeedRandomLayer::hide() {
	if (!isShown) {
...
QRectF MultiSeedRandomLayer::boundingRect() const {
...
//		}
		m_rep->getData()->seismic()->sampleTransformation()->direct(ij[2 * i + 1], y);

		QPointF outPt = dynamic_cast<RandomLineView*>(m_rep->view())->getScenePosFromDiscreatePolyLinePos(QPointF(ij[2*i], y));
		x = outPt.x();
...
void MultiSeedRandomLayer::refreshPolygons() {
...
//	}
	const AffineTransformation* sampleTransform = m_rep->getData()->seismic()->sampleTransformation();
	...
	QPolygon polygon = dynamic_cast<RandomLineView*>(m_rep->view())->discreatePolyLine();
	QTransform mainTransform = Affine2DTransformation(polygon.size(), m_rep->getData()->seismic()->height(), transform, this).imageToWorldTransformation().toTransform();
...
void MultiSeedRandomLayer::refresh() {
	refreshPolygons();
	...
	for (const std::pair<std::size_t, RgtSeed>& seed : m_rep->getData()->getMap()) {
		std::size_t idx = 0;
		QPolygon polygon = dynamic_cast<RandomLineView*>(m_rep->view())->discreatePolyLine();
		while (idx<polygon.size() && (polygon[idx].x() != seed.second.y || polygon[idx].y() != seed.second.z)) {
			idx++;
		}
		bool isPointInSection = idx<polygon.size() && polygon[idx].x() == seed.second.y && polygon[idx].y() == seed.second.z;
		if (isPointInSection) {
...
void MultiSeedRandomLayer::referencesChanged() {
	std::vector<std::shared_ptr<FixedLayerFromDataset>> references = m_rep->getData()->getReferences();
