...
WellBoreLayerOnSlice::WellBoreLayerOnSlice(WellBoreRepOnSlice *rep,QGraphicsScene *scene, int defaultZDepth,QGraphicsItem * parent) :
...
//	path.addPolygon(poly);
	...
	connect(wellBore, &WellBore::logColorChanged, this, &WellBoreLayerOnSlice::setLogColor);

	refresh();
}

WellBoreLayerOnSlice::~WellBoreLayerOnSlice() {
}

void WellBoreLayerOnSlice::show() {
...
//	m_view->installEventFilter(this);
	...
	//connect(m_scene, &QGraphicsScene::sceneRectChanged, this, &WellBoreLayerOnSlice::refreshLog);
}

void WellBoreLayerOnSlice::hide() {
//	if (m_view!=nullptr) {
...
QRectF WellBoreLayerOnSlice::boundingRect() const
{
...
void WellBoreLayerOnSlice::refresh() {
...
bool WellBoreLayerOnSlice::isShown() const {
	return m_isShown;
}

void WellBoreLayerOnSlice::setLogColor(QColor color) {
	QPen pen = m_itemLog->pen();
	pen.setColor(color);
	m_itemLog->setPen(pen);
}

void WellBoreLayerOnSlice::toggleLogDisplay(bool showLog) {
	...
	if (m_isShownLog!=showLog) {
		...
		if (m_isShownLog) {
			...
			qDebug()<<"toggleLogDisplay HIDE ";
			m_scene->removeItem(m_itemLog);
		}
	}
}

void WellBoreLayerOnSlice::refreshLog() {
	if (m_view==nullptr) {
		return;
	}
	...
	if (!m_scaleInit || std::fabs(scale-m_cachedScale)>std::min(0.001, scale/100.0)) {
		...
		// build polygon
		QPainterPath pathLog;
		const std::vector<std::vector<WellBoreRepOnSlice::LogGraphicPoint>>& logParams = m_rep->logDisplayParams();

		if (!qFuzzyIsNull(logDynamic)) {
			for (const std::vector<WellBoreRepOnSlice::LogGraphicPoint>& curveParams : logParams) {
				QPolygonF polygon;

				for (const WellBoreRepOnSlice::LogGraphicPoint& param : curveParams) {
...
//bool WellBoreLayerOnSlice::eventFilter(QObject* watched, QEvent* ev) {
...
double WellBoreLayerOnSlice::origin() const {
	return m_origin;
}

void WellBoreLayerOnSlice::setOrigin(double val) {
	if (m_origin!=val) {
		m_origin = val;
		if (m_isShownLog) {
			m_scaleInit = false;
...
double WellBoreLayerOnSlice::width() const {
	return m_width;
}

void WellBoreLayerOnSlice::setWidth(double val) {
	if (m_width!=val) {
		m_width = val;
		if (m_isShownLog) {
			m_scaleInit = false;
...
double WellBoreLayerOnSlice::logMin() const {
	return m_logMin;
}

void WellBoreLayerOnSlice::setLogMin(double val) {
	if (m_logMin!=val) {
		m_logMin = val;
		if (m_isShownLog) {
			m_scaleInit = false;
...
double WellBoreLayerOnSlice::logMax() const {
	return m_logMax;
}

void WellBoreLayerOnSlice::setLogMax(double val) {
	if (m_logMax!=val) {
		m_logMax = val;
		if (m_isShownLog) {
			m_scaleInit = false;
...
void WellBoreLayerOnSlice::setPenWidth(double val)
{
	if(m_item != nullptr)
	{
		QPen pen = m_item->pen();
