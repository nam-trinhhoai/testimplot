...
StratiSliceRGB3DLayer::StratiSliceRGB3DLayer(StratiSliceRGBAttributeRep *rep, QWindow *parent, Qt3DCore::QEntity *root,
	...
	connect(m_rep->stratiSliceAttribute()->image()->get(0), SIGNAL(dataChanged()), this,
			SLOT(updateRed()));
	connect(m_rep->stratiSliceAttribute()->image()->get(1), SIGNAL(dataChanged()), this,
			SLOT(updateGreen()));
	connect(m_rep->stratiSliceAttribute()->image()->get(2), SIGNAL(dataChanged()), this,
			SLOT(updateBlue()));
...
StratiSliceRGB3DLayer::~StratiSliceRGB3DLayer() {

}

void StratiSliceRGB3DLayer::opacityChanged(float val) {
	m_opacityParameter->setValue(val);
}

void StratiSliceRGB3DLayer::rangeChanged(unsigned int i, const QVector2D &value) {
	if (i == 0)
		m_paletteRedRangeParameter->setValue(
				m_rep->stratiSliceAttribute()->image()->get(0)->rangeRatio());
	else if (i == 1)
		m_paletteGreenRangeParameter->setValue(
				m_rep->stratiSliceAttribute()->image()->get(1)->rangeRatio());
	else if (i == 2)
		m_paletteBlueRangeParameter->setValue(
				m_rep->stratiSliceAttribute()->image()->get(2)->rangeRatio());
}

void StratiSliceRGB3DLayer::updateTexture(CudaImageTexture *texture,
...
void StratiSliceRGB3DLayer::updateRed() {
	updateTexture(m_cudaRedTexture, m_rep->stratiSliceAttribute()->image()->get(0));
	rangeChanged(0, m_rep->stratiSliceAttribute()->image()->get(0)->range());
}

void StratiSliceRGB3DLayer::updateGreen() {
	updateTexture(m_cudaGreenTexture, m_rep->stratiSliceAttribute()->image()->get(1));
	rangeChanged(1, m_rep->stratiSliceAttribute()->image()->get(1)->range());
}

void StratiSliceRGB3DLayer::updateBlue() {
	updateTexture(m_cudaBlueTexture, m_rep->stratiSliceAttribute()->image()->get(2));
	rangeChanged(2, m_rep->stratiSliceAttribute()->image()->get(2)->range());
}

void StratiSliceRGB3DLayer::updateIsoSurface() {
	updateTexture(m_cudaSurfaceTexture,
			m_rep->stratiSliceAttribute()->isoSurfaceHolder());
}

StratiSlice* StratiSliceRGB3DLayer::stratiSlice() const {
	return m_rep->stratiSliceAttribute()->stratiSlice();
}

void StratiSliceRGB3DLayer::show() {
	...
	CUDAImagePaletteHolder *img = m_rep->stratiSliceAttribute()->image()->get(0);
	m_cudaRedTexture = new CudaImageTexture(ImageFormats::QColorFormat::GRAY,
			img->sampleType(), img->width(), img->height());

	img = m_rep->stratiSliceAttribute()->image()->get(1);
	m_cudaGreenTexture = new CudaImageTexture(ImageFormats::QColorFormat::GRAY,
			img->sampleType(), img->width(), img->height());

	img = m_rep->stratiSliceAttribute()->image()->get(2);
	m_cudaBlueTexture = new CudaImageTexture(ImageFormats::QColorFormat::GRAY,
			img->sampleType(), img->width(), img->height());
...
void StratiSliceRGB3DLayer::zScale(float val)
{
	m_transform->setScale3D(QVector3D(1, val, 1));
}

void StratiSliceRGB3DLayer::hide() {
...
QRect3D StratiSliceRGB3DLayer::boundingRect() const {
...
void StratiSliceRGB3DLayer::refresh() {
