...
FixedLayersFromDatasetAndCubePropPanel::FixedLayersFromDatasetAndCubePropPanel(FixedLayersFromDatasetAndCubeRep *rep, QWidget *parent, bool enable) :
		QWidget(parent) {
	if ( !enable) return;
	m_rep = rep;
	...
	connect(m_modeComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &FixedLayersFromDatasetAndCubePropPanel::modeChangedInternal);
	...
	connect(m_slider, &QSlider::valueChanged, this, &FixedLayersFromDatasetAndCubePropPanel::changeDataKeyFromSlider);
	...
	connect(m_layerNameSpinBox, &EditingSpinBox::contentUpdated, this, &FixedLayersFromDatasetAndCubePropPanel::changeDataKeyFromSpinBox,
			Qt::QueuedConnection);
	...
	connect(m_multiplierComboBox, QOverload<int>::of(&QComboBox::currentIndexChanged), this, &FixedLayersFromDatasetAndCubePropPanel::multiplierChanged);
...
//	holderLayerLayoutLv2->addWidget(m_multiplierComboBox, 0, Qt::AlignmentFlag::AlignLeft);
	...
	connect(m_rep->fixedLayersFromDataset(), &FixedLayersFromDatasetAndCube::modeChanged, this,
			&FixedLayersFromDatasetAndCubePropPanel::modeChanged);
	...
	connect(m_rep->fixedLayersFromDataset(), &FixedLayersFromDatasetAndCube::initProgressBar, this,
			&FixedLayersFromDatasetAndCubePropPanel::initProgressBar);
	connect(m_rep->fixedLayersFromDataset(), &FixedLayersFromDatasetAndCube::valueProgressBarChanged, this,
			&FixedLayersFromDatasetAndCubePropPanel::valueProgressBarChanged);
	connect(m_rep->fixedLayersFromDataset(), &FixedLayersFromDatasetAndCube::endProgressBar, this,
			&FixedLayersFromDatasetAndCubePropPanel::endProgressBar);

	connect(m_lockPalette, &QCheckBox::stateChanged, this,
			&FixedLayersFromDatasetAndCubePropPanel::lockPalette);
	connect(m_palette, &PaletteWidget::rangeChanged,
				this, &FixedLayersFromDatasetAndCubePropPanel::updateLockRange);
	connect(m_palette, &PaletteWidget::lookupTableChanged,
				this, &FixedLayersFromDatasetAndCubePropPanel::updateLockLookupTable);

	connect(m_rep->fixedLayersFromDataset()->image(),
			QOverload<const QVector2D&>::of(&CPUImagePaletteHolder::rangeChanged), this,
			&FixedLayersFromDatasetAndCubePropPanel::updateLockRange);
	connect(m_rep->fixedLayersFromDataset()->image(),
			QOverload<const LookupTable&>::of(&CPUImagePaletteHolder::lookupTableChanged), this,
			&FixedLayersFromDatasetAndCubePropPanel::updateLockLookupTable);

	modeChanged(); // finish init
}

FixedLayersFromDatasetAndCubePropPanel::~FixedLayersFromDatasetAndCubePropPanel() {
}

void FixedLayersFromDatasetAndCubePropPanel::changeDataKeyFromSlider(long index) {
	if (m_mutex.tryLock()) {
		{
...
void FixedLayersFromDatasetAndCubePropPanel::changeDataKeyFromSliderInternal(long index) {
...
void FixedLayersFromDatasetAndCubePropPanel::changeDataKeyFromSpinBox() {
...
void FixedLayersFromDatasetAndCubePropPanel::multiplierChanged(int index) {
	bool ok;
...
void FixedLayersFromDatasetAndCubePropPanel::updatePalette() {
	if (m_palette) {
		m_palette->setPaletteHolder(m_rep->fixedLayersFromDataset()->image());
	}
}

void FixedLayersFromDatasetAndCubePropPanel::modeChangedInternal(int index) {
...
void FixedLayersFromDatasetAndCubePropPanel::modeChanged() {
	FixedLayersFromDatasetAndCube* data = m_rep->fixedLayersFromDataset();
...
void FixedLayersFromDatasetAndCubePropPanel::initProgressBar(int min, int max, int val) {
	m_progressBar->setRange(min, max);
	m_progressBar->setValue(val);
	m_progressBar->show();
}

void FixedLayersFromDatasetAndCubePropPanel::valueProgressBarChanged(int val) {
	m_progressBar->setValue(val);
}

void FixedLayersFromDatasetAndCubePropPanel::endProgressBar() {
	m_progressBar->hide();
}

void FixedLayersFromDatasetAndCubePropPanel::updateLockCheckBox() {
...
void FixedLayersFromDatasetAndCubePropPanel::lockPalette(int state) {
	FixedLayersFromDatasetAndCube* data = m_rep->fixedLayersFromDataset();
	if (state==Qt::Checked) {
...
void FixedLayersFromDatasetAndCubePropPanel::updateLockRange(const QVector2D & range) {
	FixedLayersFromDatasetAndCube* data = m_rep->fixedLayersFromDataset();
...
void FixedLayersFromDatasetAndCubePropPanel::updateLockLookupTable(const LookupTable& lookupTable) {
	FixedLayersFromDatasetAndCube* data = m_rep->fixedLayersFromDataset();
	if (data->isPaletteLocked()) {
