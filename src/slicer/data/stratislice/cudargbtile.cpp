...
CUDARGBTile::CUDARGBTile(int w, int h, int tileSize, int window) {
	m_w = w;
	m_h = h;
	m_tileSize = tileSize;
	m_window = window;
...
//state 1
void CUDARGBTile::run(short z, unsigned int f1, unsigned int f2,
		unsigned int f3, short *isoVal, float *f1Val, float *f2Val,
		float *f3Val) {
	...
	optimizedFFTModule(stream, deviceOutputData, f1Val + m_d0 * m_w,
			f2Val + m_d0 * m_w, f3Val + m_d0 * m_w, m_w, m_h, m_depth, m_window,
			f1, f2, f3);
}

void CUDARGBTile::fillHostBuffer(Seismic3DDataset *seismic, int channelS,
