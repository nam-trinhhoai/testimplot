...
ScalarComputationOnDataset::ScalarComputationOnDataset(Seismic3DAbstractDataset* dataset, int channel, WorkingSetManager *workingSet,
	...
	m_repFactory = new ScalarComputationOnDatasetGraphicRepFactory(this);
}
ScalarComputationOnDataset::~ScalarComputationOnDataset() {
	delete m_repFactory;
}

const AffineTransformation * const ScalarComputationOnDataset::sampleTransformation() const {
	return m_sampleTransformation.get();
}

const Affine2DTransformation * const ScalarComputationOnDataset::ijToXYTransfo() const {
	return m_ijToXY.get();
}

const Affine2DTransformation * const ScalarComputationOnDataset::ijToInlineXlineTransfo() const {
	return m_ijToInlineXline.get();
}

const Affine2DTransformation * const ScalarComputationOnDataset::ijToInlineXlineTransfoForInline() const {
	return m_ijToInlineXlineForInline.get();
}

const Affine2DTransformation * const ScalarComputationOnDataset::ijToInlineXlineTransfoForXline() const {
...
void ScalarComputationOnDataset::computeSpectrumOnInlineXLine(SliceDirection dir,
...
void ScalarComputationOnDataset::computeSpectrumOnRandom(const QPolygon& randomLine, short** buffer) {
	std::unique_ptr<CUDAImagePaletteHolder> holder;
	holder.reset(new CUDAImagePaletteHolder(
...
void ScalarComputationOnDataset::loadInlineXLine(CUDAImagePaletteHolder *cudaImage,
...
void ScalarComputationOnDataset::loadRandomLine(CUDAImagePaletteHolder *cudaImage,
...
IGraphicRepFactory *ScalarComputationOnDataset::graphicRepFactory() {
	return m_repFactory;
}

//IData
QUuid ScalarComputationOnDataset::dataID() const {
	return m_uuid;
}

SeismicSurvey* ScalarComputationOnDataset::survey() const {
	return m_dataset->survey();
}

QRectF ScalarComputationOnDataset::inlineXlineExtent() const {
	return m_ijToInlineXline->worldExtent();
}

LookupTable ScalarComputationOnDataset::defaultLookupTable() const {
	return ColorTableRegistry::DEFAULT();
}

CubeSeismicAddon ScalarComputationOnDataset::cubeSeismicAddon() const {
	return m_seismicAddon;
}

ArraySpectralImageCache* ScalarComputationOnDataset::createInlineXLineCache(SliceDirection dir) const {
...
ArraySpectralImageCache* ScalarComputationOnDataset::createRandomCache(const QPolygon& poly) const {
	return new ArraySpectralImageCache(poly.size(), m_height, m_dimV, m_sampleType);
}

double ScalarComputationOnDataset::getFrequency(long fIdx) const {
	return getFrequencyStatic(fIdx, m_sampleTransformation->a(), m_windowSize);
}

double ScalarComputationOnDataset::getFrequencyStatic(long freqIndex, double pasech, long windowSize)  {
