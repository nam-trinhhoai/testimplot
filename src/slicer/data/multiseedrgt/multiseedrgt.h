#ifndef MultiSeedRgt_H
#define MultiSeedRgt_H
...
class MultiSeedRgt : public EditableData {
Q_OBJECT
public:
	MultiSeedRgt(QString name, WorkingSetManager *workingSet,
			Seismic3DDataset *seismic, int channelSeismic,
			Seismic3DDataset *rgt, int channelRgt, QObject *parent = 0);
	virtual ~MultiSeedRgt();
...
private:
	...
	struct UpdateSeedsPositionWithReadKernel {
		static void run(MultiSeedRgt* obj, std::vector<std::tuple<RgtSeed, RgtSeed, std::size_t>>& vect);
	};
...
Q_DECLARE_METATYPE(MultiSeedRgt*)
#endif
