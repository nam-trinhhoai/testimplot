...
RGBLayerSlice::RGBLayerSlice(WorkingSetManager *workingSet,
	...
	m_repFactory.reset(new RGBLayerSliceGraphicRepFactory(this));
	resetFrequencies();

	connect(m_layerSlice, &LayerSlice::computationFinished, this,
		&RGBLayerSlice::updateFromComputation);
	connect(m_layerSlice, &LayerSlice::deleteRgtLayer, this,
			&RGBLayerSlice::deleteRep);

	m_decorator = nullptr;
}

RGBLayerSlice::~RGBLayerSlice() {

}


int RGBLayerSlice::redIndex() const {
	return m_freqIndex[0];
}

int RGBLayerSlice::greenIndex() const {
	return m_freqIndex[1];
}

int RGBLayerSlice::blueIndex() const {
	return m_freqIndex[2];
}

void RGBLayerSlice::setRedIndex(int value) {
	m_freqIndex[0] = value;
	loadSlice();
	if (m_locked) {
...
void RGBLayerSlice::setGreenIndex(int value) {
	m_freqIndex[1] = value;
	loadSlice();
	if (m_locked) {
...
void RGBLayerSlice::setBlueIndex(int value) {
	m_freqIndex[2] = value;
	loadSlice();
	if (m_locked) {
...
void RGBLayerSlice::setRGBIndexes(int red, int green, int blue) {
	m_freqIndex[0] = red;
	m_freqIndex[1] = green;
	m_freqIndex[2] = blue;
	loadSlice();
...
QUuid RGBLayerSlice::dataID() const {
return m_layerSlice->dataID();
}

QString RGBLayerSlice::name() const {
	return "RGB " + m_layerSlice->name();
}

//IData
IGraphicRepFactory* RGBLayerSlice::graphicRepFactory() {
	return m_repFactory.get();
}

void RGBLayerSlice::loadSlice() {
...
void RGBLayerSlice::resetFrequencies() {
...
// original
int RGBLayerSlice::getMaximumEnergyComponent() {
	int bestComponent = 0;
	int bestRange = 0;
	std::size_t N = m_image->width() * m_image->height();
...
int RGBLayerSlice::getMaximumEnergyComponent(float *tab, size_t size) {
...
void RGBLayerSlice::deleteRep() {
    emit deletedRep();
}

bool RGBLayerSlice::isLocked() const {
	return m_locked;
}

int RGBLayerSlice::lockedRedIndex() const {
	return m_lockedRedIndex;
}

int RGBLayerSlice::lockedGreenIndex() const {
	return m_lockedGreenIndex;
}

int RGBLayerSlice::lockedBlueIndex() const {
	return m_lockedBlueIndex;
}

QVector2D RGBLayerSlice::lockedRedRange() const {
	return m_lockedRedRange;
}

QVector2D RGBLayerSlice::lockedGreenRange() const {
	return m_lockedGreenRange;
}

QVector2D RGBLayerSlice::lockedBlueRange() const {
	return m_lockedBlueRange;
}

void RGBLayerSlice::setLockedRedRange(const QVector2D& redRange) {
...
void RGBLayerSlice::setLockedGreenRange(const QVector2D& greenRange) {
...
void RGBLayerSlice::setLockedBlueRange(const QVector2D& blueRange) {
...
void RGBLayerSlice::updateFromComputation() {
	if (m_locked) {
		loadSlice();
	} else {
		resetFrequencies();
...
bool RGBLayerSlice::isMinimumValueActive() const {
	return m_useMinimumValue;
}

void RGBLayerSlice::setMinimumValueActive(bool active, bool bypassLock) {
...
float RGBLayerSlice::minimumValue() const {
	return m_minimumValue;
}

void RGBLayerSlice::setMinimumValue(float minimumValue, bool bypassLock) {
...
IsoSurfaceBuffer RGBLayerSlice::getIsoBuffer()
{
	return m_layerSlice->getIsoBuffer();
}

ITreeWidgetItemDecorator* RGBLayerSlice::getTreeWidgetItemDecorator() {
