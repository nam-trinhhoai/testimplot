...
namespace QtImGui {
    ...
    // save list font file to vector
    int ImGuiRenderer::listDir(const char* dirname) {
        DIR* dir;
        ...
        if (dir == NULL) {
            ...
            while ((subdir = readdir(dir)) != NULL) {
                if (strstr(subdir->d_name, ".ttf")) {
                    font_files.push_back(std::string(subdir->d_name));
                }
            }

            for (int i = 0; i < font_files.size(); i++) {
                std::cout << font_files[i];
            }
            closedir(dir);
            return 0;
        }
    }
    void ImGuiRenderer::initFont() {
        if (!isFontInited) {
            char cwd[1024]{};
            float fontSize = 16.0f;
            getcwd(cwd, sizeof(cwd));
            std::string current_path(cwd);
            const std::string path_font = current_path + "/bin/palettes/fonts/";
            listDir(path_font.c_str());
            ImGuiIO& io = ImGui::GetIO();
            char* font_default = "Roboto-Medium.ttf";

            ImFileHandle f;

            if ((f = ImFileOpen((path_font + font_default).c_str(), "rb")) != NULL) {
                io.Fonts->AddFontFromFileTTF((path_font + font_default).c_str(), fontSize);
                ImFileClose(f);
            }
            //add others fonts
            for (int i = 0; i < font_files.size(); i++) {
                if (strstr(font_files[i].c_str(), font_default)) {
                    continue;
                }
                if ((f = ImFileOpen((path_font + font_files[i]).c_str(), "rb")) != NULL) {
                    io.Fonts->AddFontFromFileTTF((path_font + font_files[i]).c_str(), fontSize);
                    ImFileClose(f);
                }
