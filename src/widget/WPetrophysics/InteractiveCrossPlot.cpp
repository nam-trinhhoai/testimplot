...
InteractiveCrossPlot::InteractiveCrossPlot(WorkingSetManager* manager):
m_manager(manager)
{}

InteractiveCrossPlot::~InteractiveCrossPlot()
{}

// Interactive helper			
int InteractiveCrossPlot::interactiveHelper(double *log1, double *log2, int numPoints)
{
...
void InteractiveCrossPlot::showPlot() {
		...
		// Plot
		{
			...
			if (ImPlot::BeginPlot((logName1+" vs "+logName2).c_str(), ImVec2(-1,-1))) 
			{
				...
				if ((currentLog1.attributes.size()>0) & (currentLog2.attributes.size()>0))
				{
					...
					int idx = interactiveHelper(log1, log2, numPoints);
					ImPlot::PlotScatterHighlight(idx, (logName1+" vs "+logName2).c_str(), log1, log2, numPoints);	
				}
