...
PlotHistogramCurve::PlotHistogramCurve(WorkingSetManager* manager):
m_manager(manager)
{}

PlotHistogramCurve::~PlotHistogramCurve()
{}

// Cross-plot between the logs
void PlotHistogramCurve::showPlot() {
		...
		// Plot histogram
		{
			...
			if (ImPlot::BeginPlot("##Histograms", ImVec2(-1, -1))) {
				...
				if (currentLog1.attributes.size()>0)
					ImPlot::PlotHistogramCurve(logName1.c_str(), log1, numPoints, bins, cumulative, density, range ? ImPlotRange(rmin,rmax) : ImPlotRange(), outliers);
