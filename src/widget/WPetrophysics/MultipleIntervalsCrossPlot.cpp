...
MultipleIntervalsCrossPlot::MultipleIntervalsCrossPlot(WorkingSetManager* manager):
m_manager(manager)
{}

MultipleIntervalsCrossPlot::~MultipleIntervalsCrossPlot()
{}

// Plot the well logs from database in quickplot format
void MultipleIntervalsCrossPlot::showPlot() {
	...
	if(totalNumberOfWellBores > 0)
	{
		...
		if(currentLog1.attributes.size() > 0)
		{
			...
			static double depthMin = 10000;
			static double depthMax = 0;
...
std::cout << "depthMin" << std::to_string(depthMin) << std::endl;
std::cout << "depthMax" << std::to_string(depthMax) << std::endl;
			...
			static double selectedTop[3] = {depthMin,2000,2000};  // Temporal solution
