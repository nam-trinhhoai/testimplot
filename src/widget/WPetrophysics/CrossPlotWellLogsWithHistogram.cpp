...
CrossPlotWellLogsWithHistogram::CrossPlotWellLogsWithHistogram(WorkingSetManager* manager):
m_manager(manager)
{}

CrossPlotWellLogsWithHistogram::~CrossPlotWellLogsWithHistogram()
{}


// Cross-plot between the logs
void CrossPlotWellLogsWithHistogram::showPlot() {
			...
			if (ImGui::BeginTable("##table", 2, table_flags, ImVec2(-1,-1))) 
			{
				...
				if (ImPlot::BeginPlot("##Histograms1", ImVec2(-1, 200))) {
					...
					if (currentLog1.attributes.size()>0)
						ImPlot::PlotHistogram(logName1.c_str(), log1, numPoints, 100, false, false, ImPlotRange(), true);
				...
				if (ImPlot::BeginPlot("##Histograms2", ImVec2(-1, -1))) {
					...
					if (currentLog2.attributes.size()>0)
						ImPlot::PlotHistogramH(logName2.c_str(), log2, numPoints, 100, false, false, ImPlotRange(), true);
