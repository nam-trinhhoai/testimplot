...
InteractiveMultipleWindows::InteractiveMultipleWindows(WorkingSetManager* manager):
m_manager(manager)
{}

InteractiveMultipleWindows::~InteractiveMultipleWindows()
{}

// Main code
void InteractiveMultipleWindows::showPlot()
{	
	showHistogram();	
}

// Interactive histogram helper			
std::tuple<int, int, std::vector<double>, double, double> InteractiveMultipleWindows::interactiveHistogramHelper(double *log, int numPoints, int bins)
{
	double logMin = 1e6;
	double logMax = 1e-6;
...
// Histogram
void InteractiveMultipleWindows::showHistogram() {
	// We demonstrate using the full viewport area or the work area (without menu-bars, task-bars etc.)
	// Based on your use case you may want one of the other.
	static bool use_work_area = false;
	const ImGuiViewport* viewport = ImGui::GetMainViewport();
	//ImGui::SetNextWindowPos(use_work_area ? viewport->WorkPos : viewport->Pos);
	ImGui::SetNextWindowSize(use_work_area ? viewport->WorkSize : ImVec2(600,400));
	...
	if(totalNumberOfWellBores > 0)
	{
		...
		// Plot histogram
		{
			...
			ImGui::Begin("ChildRight", &showPlot, winflags);
			...
			if (ImPlot::BeginPlot("##Histograms", ImVec2(-1, -1))) {
				...
				if (currentLog1.attributes.size()>0)
				{
					...
					std::tie(idx, countPointsInActiveBin, activePoints, xActiveBinMin, xActiveBinMax) = interactiveHistogramHelper(log1, numPoints, bins);
